...
describe('Redis Operations', function () {
  // Increase the timeout if Redis operations take longer
  this.timeout(5000);

  it('+ Setting and getting an expired value', async function () {
    await redisClient.set('test_key', '356', 1); // Set key to expire in 1 second
    // Wait for a bit more than 1 second to ensure the key has expired
  ...
  it('+ Setting and getting a deleted value', async function () {
    ...
    const value = await redisClient.get('test_key');
    // The key should be null since it was deleted
